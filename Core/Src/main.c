...
PID_TypeDef TPID;
double tpsPercent = 0, pedalPercent = 0;
...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
	...
	uint16_t pedal_value = 0, tps1_raw = 0;
	float tps1_voltage = 0;
  ...
  while (1)
  {
	  ...
	  pwmValue = (uint32_t) PIDOut;

	  htim3.Instance->CCR1 = pedal_value;

	  HAL_Delay(100);

	  // htim3.Instance->CCR1 = adc_convertions[2];
